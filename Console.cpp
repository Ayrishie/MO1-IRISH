...
void Console::initialize() {
    ...
    if (fs::is_directory(logDir)) {
        ...
        // Scheduler init
        if (schedulerType == "rr") {
            rrScheduler = std::make_unique<RRScheduler>(cpuCount, timeQuantum, delayPerExecution, pagingAllocator.get());
        }
...
bool Console::memoryCheck(int maxMem, int frameSize, int minMemPerProc, int maxMemPerProc) {
    ...
    if (!isPowerOfTwo(maxMem) || !isPowerOfTwo(frameSize) || !isPowerOfTwo(minMemPerProc) || !isPowerOfTwo(maxMemPerProc) {
...
int Console::generateProcessSize() {
    ...
    std::uniform_int_distribution<int> distribution(this->minMemPerProc, this->maxMemPerProc);
